...
const mataDepth = translateStata(String.raw`mata
real matrix A, B
real scalar i
A=(1,2\3,.)
B=J(2,2,1)
...
A=editmissing(A,0)
for (i=1; i<=rows(A); i++) {
  if (A[i,1]>0) A[i,2] = A[i,2] + 1
}
st_matrix("M", A)
st_numscalar("answer", min(A))
st_local("tag", "done")
end`);
assert.equal(mataDepth.counts.review, 0);
assert.match(mataDepth.code, /A <- A %\*% B/);
assert.match(mataDepth.code, /m <- mata_rowminmax\(A\)/);
assert.match(mataDepth.code, /cm <- matrix\(colSums\(is\.na\(as\.matrix\(A\)\)\), nrow = 1L\)/);
assert.match(mataDepth.code, /A <- mata_editmissing\(A, 0\)/);
assert.match(mataDepth.code, /for \(i in seq\(from = 1, to = nrow\(A\), by = 1\)\)/);
assert.match(mataDepth.code, /if \(A\[i,1\]>0\) \{/);
assert.match(mataDepth.code, /mata_st_matrix\("M", A\)/);
assert.match(mataDepth.code, /mata_st_numscalar\("answer", mata_min\(A\)\)/);
assert.match(mataDepth.code, /stata_local_set\(\.do2r_local, "tag", "done"\)/);
assert.match(mataDepth.code, /mata_apply_ignore_missing/);

const matrixProgramming = translateStata(String.raw`matrix A = (1,2\3,4)
matrix B = A * A'
matrix K = A # I(2)
matrix rownames B = r1 r2
matrix colnames B = c1 c2
matrix roweq B = eq1 eq2
matrix coleq B = eq1 eq2
matrix list B
mkmat x y if keep, matrix(X) nomissing obs
svmat double X, names(col)
matrix accum XX = x y [aw=w] if keep, deviations
matrix vecaccum XY = y x [fw=w] if keep, noconstant
matrix score xb = b if keep, replace forcezero`);
assert.equal(matrixProgramming.counts.review, 0);
assert.match(matrixProgramming.code, /A <- rbind\(c\(1, 2\), c\(3, 4\)\)/);
assert.match(matrixProgramming.code, /B <- A %\*% t\(A\)/);
assert.match(matrixProgramming.code, /K <- kronecker\(A, diag\(2\)\)/);
assert.match(matrixProgramming.code, /attr\(B, "stata_row_eq"\)/);
assert.match(matrixProgramming.code, /stata_mkmat\(dt, vars = c\("x", "y"\)/);
assert.match(matrixProgramming.code, /stata_svmat\(dt, X/);
assert.match(matrixProgramming.code, /stata_matrix_accum/);
assert.match(matrixProgramming.code, /vec = TRUE/);
assert.match(matrixProgramming.code, /stata_matrix_score\(dt, b, forcezero = TRUE\)/);

const putGetMata = translateStata(`putmata X=(x y 1) v=z if keep, omitmissing replace
getmata (x2 y2)=X, force replace
getmata score=v, update`);
assert.equal(putGetMata.counts.review, 0);
assert.match(putGetMata.code, /stata_putmata_rows/);
assert.match(putGetMata.code, /stata_putmata_matrix/);
assert.match(putGetMata.code, /K_views = 0L, K_copies/);
assert.match(putGetMata.code, /stata_getmata/);
assert.match(putGetMata.code, /targets = c\("x2", "y2"\), value = X/);
assert.match(putGetMata.code, /update = TRUE/);
assert.match(putGetMata.code, /K_new/);

const mataManagement = translateStata(`mata clear
mata: A=(1,2)
mata describe A
mata rename A B
mata drop B`);
assert.equal(mataManagement.counts.review, 0);
assert.match(mataManagement.code, /mata clear: translated Mata type state reset/);
assert.match(mataManagement.code, /B <- A/);
assert.match(mataManagement.code, /rm\(list = intersect/);
...
const unitRootDepth = translateStata(`tsset t
dfuller y if sample, lags(2)
dfgls y if sample, maxlag(4) notrend
pperron y if sample, trend lags(0) regress
wntestq y if sample, lags(12)`);
assert.equal(unitRootDepth.counts.review, 0);
assert.match(unitRootDepth.code, /urca::ur\.df\(dt\[sample\]\[\["y"\]\]/);
assert.match(unitRootDepth.code, /urca::ur\.ers/);
assert.match(unitRootDepth.code, /model = "constant"/);
assert.match(unitRootDepth.code, /stata_pperron_zero/);
assert.match(unitRootDepth.code, /if \(\.__do2r_pp_k_4 == 0L\)/);
assert.match(unitRootDepth.code, /stats::Box\.test/);
assert.match(unitRootDepth.code, /type = "Ljung-Box"/);
...
const cloglogPanel = translateStata(`xtset id t
xtcloglog y x i.g, offset(off) intpoints(8)
xtcloglog y x, pa`);
assert.equal(cloglogPanel.counts.review, 0);
assert.match(cloglogPanel.code, /lme4::glmer/);
assert.match(cloglogPanel.code, /binomial\(link = "cloglog"\)/);
assert.match(cloglogPanel.code, /nAGQ = 8L/);
assert.match(cloglogPanel.code, /geepack::geeglm/);
...
const egenStats = translateStata(`egen g = group(a b) if keep
egen tag = tag(a b) if keep
by sector: egen i = iqr(x) if keep
by sector: egen sk = skew(x)
by sector: egen ku = kurt(x)
by sector: egen md = mad(x)
by sector: egen dv = mdev(x)
by sector: egen p90 = pctile(x), p(90)
by sector: egen mo = mode(cat), minmode
by sector: egen z = std(x), mean(10) sd(2)
by sector: egen r = rank(x), field
egen c = cut(x), at(0(5)20) icodes
egen q = cut(x), group(4)`, { addHeader: false });
assert.equal(egenStats.counts.review, 0);
assert.match(egenStats.code, /stata_egen_group/);
assert.match(egenStats.code, /stata_egen_tag/);
assert.match(egenStats.code, /stata_egen_skew/);
assert.match(egenStats.code, /stata_egen_kurt/);
assert.match(egenStats.code, /probs = c\(0\.25, 0\.75\)/);
assert.match(egenStats.code, /probs = \(90\) \/ 100/);
assert.match(egenStats.code, /stata_egen_mode/);
assert.match(egenStats.code, /\(10\) \+ \(2\) \* \(\.z - \.m\) \/ \.s/);
assert.match(egenStats.code, /ties\.method = "min"/);
assert.match(egenStats.code, /stata_egen_cut\(x, at = stata_numlist\("0\(5\)20"\)/);
assert.match(egenStats.code, /groups = 4/);

const richTabulate = translateStata(`tabulate group if keep, missing sort generate(g) matcell(C) matrow(R)
tabulate row col [fw=w] if keep, row col cell expected cchi2 clrchi2 chi2 exact lrchi2 V gamma taub rowsort matcell(M) matrow(MR) matcol(MC)
by region: tabulate status, missing`);
assert.equal(richTabulate.counts.review, 0);
assert.match(richTabulate.code, /stata_tabulate_oneway/);
assert.match(richTabulate.code, /stata_tabulate_generate/);
assert.match(richTabulate.code, /frequency = sum\(\.w/);
assert.match(richTabulate.code, /stata_tabulate_twoway/);
assert.match(richTabulate.code, /stats::chisq\.test/);
assert.match(richTabulate.code, /stats::fisher\.test/);
assert.match(richTabulate.code, /cramer_v/);
assert.match(richTabulate.code, /gamma = assoc\$gamma/);
assert.match(richTabulate.code, /tau_b = assoc\$tau_b/);
assert.match(richTabulate.code, /M <- \.__do2r_tab2_2\$matrix/);
assert.match(richTabulate.code, /by = \.\(region\)/);

const commonDiagnostics = translateStata(`spearman x y z if keep, stats(rho p obs) pw sidak
alpha q1 q2 q3 if keep, std item generate(scale)
ranksum y if keep, by(g)
signrank before = after if keep, exact
signtest before = after if keep
kwallis y if keep, by(g)
misstable summarize x y, all generate(m_, exok)
misstable patterns x y, frequency bypatterns exok`);
assert.equal(commonDiagnostics.counts.review, 0);
assert.match(commonDiagnostics.code, /stata_spearman/);
assert.match(commonDiagnostics.code, /adjust = "sidak"/);
assert.match(commonDiagnostics.code, /psych::alpha/);
assert.match(commonDiagnostics.code, /correct = FALSE/);
assert.match(commonDiagnostics.code, /stats::pbinom/);
assert.match(commonDiagnostics.code, /stats::kruskal\.test/);
assert.match(commonDiagnostics.code, /stata_misstable_summary/);
assert.match(commonDiagnostics.code, /stata_misstable_patterns/);
assert.match(commonDiagnostics.code, /Packages used: data\.table,[^\n]*psych/);
...
assert.match(indexHtml, /v0\.14\.0/);
assert.match(appJs, /editor-utils\.js\?v=0\.14\.0/);
...
assert.ok(COVERAGE_ROADMAP.added.some(x => /Time-series data/.test(x.family) && /dfgls/.test(x.commands) && /pperron/.test(x.commands) && /wntestq/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /nonlinear panel/.test(x.family) && /xtcloglog/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /Frequency tables/.test(x.family) && /gamma/.test(x.commands) && /taub/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /Rank tests/.test(x.family) && /signrank/.test(x.commands) && /alpha/.test(x.commands) && /misstable/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /Frequency tables/.test(x.family) && /tab1\/tab2/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /Stata matrix programming/.test(x.family) && /matrix accum/.test(x.commands) && /mkmat/.test(x.commands) && /svmat/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.added.some(x => /Mata matrix programming/.test(x.family) && /putmata\/getmata/.test(x.commands) && /st_view/.test(x.commands)));
assert.ok(COVERAGE_ROADMAP.next.some(x => /Deep Mata/.test(x.family) && /optimize/.test(x.commands) && /structs/.test(x.commands)));
